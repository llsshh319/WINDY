...
from utilss.main_utils import check_dir, collect_env, print_log, output_namespace
...
class EpochEndCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if avg_val_loss is None:
            # try epoch-scoped name used by Lightning sometimes
        ...
        if hasattr(self, 'avg_train_loss'):
            if avg_val_loss is not None:
                ...
                print_log(f"Epoch {trainer.current_epoch}: Lr: {lr:.7f} | Train Loss: {self.avg_train_loss:.7f} | Vali Loss: N/A")
